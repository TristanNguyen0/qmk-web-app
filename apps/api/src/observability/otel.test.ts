 ...
 * lets an exporter failure escape as a thrown error.
 */
...
describe('startTelemetry', () => {
  ...
  it('recording against a meter obtained while disabled is a no-op that never throws', () => {
    startTelemetry({ env: {} });
    const counter = metrics.getMeter('qwa-api').createCounter('test.disabled.counter');
  ...
  it('routes a rejecting exporter to a warn log without throwing out of a recording call', async () => {
    ...
    const counter = metrics.getMeter('qwa-api').createCounter('test.rejecting.counter');
