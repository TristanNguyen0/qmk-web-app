/**
 * Idempotent OpenTelemetry metrics bootstrap for the API process.
 *
 ...
 * *record* a metric (see `apps/api/src/observability/metrics.ts`'s header for how the
 * metrics module itself avoids binding to a stale provider). Traces stay an additive
 * next step behind this same module — see `docs/runbooks/observability.md`.
 *
 * Kept as a small file mirrored in `services/worker/src/observability/otel.ts` rather
 * than shared from one package, so neither process gains a dependency on the other's
 ...
 * a disabled handle and constructs nothing: no collector is required to run the app,
 * `pnpm dev`, or the test suite.
 */
