...
| Artifact storage | S3-compatible object storage (MinIO in dev) | Signed URLs and retention policies without exposing storage keys or worker paths. | All access goes through the artifact service; storage keys never leave the server. |
